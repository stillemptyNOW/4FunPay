...
access_granted = "🔓 Доступ к ПУ предоставлен!\n\n" \
                 ...
                 "⚙️ Чтобы открыть меню настроек <i>{{BOT_NAME}}</i>, введи команду /menu."
...
✅ <i>{{BOT_SHORT_NAME}}</i> получает полную историю чата и видит все данные о всех новых сообщениях.
✅ <i>{{BOT_SHORT_NAME}}</i> может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
✅ <i>{{BOT_SHORT_NAME}}</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>{{BOT_SHORT_NAME}}</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>{{BOT_SHORT_NAME}}</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>{{BOT_SHORT_NAME}}</i> увидит только последнее.
❌ <i>{{BOT_SHORT_NAME}}</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>{{BOT_SHORT_NAME}}</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>{{BOT_SHORT_NAME}}</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>{{BOT_SHORT_NAME}}</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>{{BOT_SHORT_NAME}}</i> сможет определить, кто именно автор сообщений."""
...
❌ <i>{{BOT_NAME}}</i> еще <b><u>не инициализирован</u></b> и <b><u>никакие его функции не работают</u></b>.\n
🔃 Как только <i>{{BOT_NAME}}</i> инициализируется, данное сообщение изменится.\n
📋 Если <i>{{BOT_SHORT_NAME}}</i> долго не инициализируется, проверь логи с помощью команды /logs"""

fpc_init = """✅ <b><u>{{BOT_NAME}} запущен и готов к работе.</u></b>\n
ℹ️ <b><i>Версия:</i></b> <code>{}</code>
👑 <b><i>Аккаунт:</i></b>  <code>{}</code> | <code>{}</code>
💰 <b><i>Баланс:</i></b> <code>{}₽, {}$, {}€</code>
📊 <b><i>Активные заказы:</i></b>  <code>{}</code>

⚙️ Настройки - команда /menu
📋 Логи - команда /logs"""
...
Автоматизация продаж на FunPay: выдача товаров, автоответы, поднятие лотов,
управление из Telegram.

<i>Основано на открытом коде {{BOT_NAME}} - см. NOTICE.md в репозитории.</i>"""

sys_info = """<b><u>Сводка данных</u></b>

<b>ЦП:</b>
{}
    Используется <i>{{BOT_SHORT_NAME}}</i>: <code>{}%</code>
...
act_edit_watermark = "Отправь новый текст водяного знака. Он подставляется в начало каждого " \
                     "сообщения, которое бот пишет покупателю.\n\n" \
                     "Текущий: {}\n\n" \
                     "Примеры (нажми, чтобы скопировать):\n" \
                     "<code>4FunPay</code>\n" \
                     "<code>[4FunPay]</code>\n" \
                     "<code>𝟒𝐅𝐮𝐧𝐏𝐚𝐲</code>\n" \
                     "<code>𝟰𝗙𝘂𝗻𝗣𝗮𝘆</code>\n" \
                     ...
                     "<code>🤖</code>\n" \
                     "<code>⚡</code>\n" \
                     "\nЭмодзи на FunPay выглядят иначе, чем в Telegram - проверь результат в чате.\n\n" \
                     "Чтобы убрать водяной знак совсем, отправь <code>-</code>."
...
backup_ready = "✅ Резервная копия конфигов, хранилища и плагинов <code>backup.zip</code>.\n\n" \
...
backup_error = "❌ Не удалось создать бэкап конфигов, хранилища и плагинов."
backup_send_error = "❌ Не удалось отправить резервную копию."
backup_not_found = "❌ Резервная копия не обнаружена."
...
pl_file_not_found_err = "❌ Не удалось обнаружить файл <code>{}</code>.\nПерезагрузите <i>{{BOT_SHORT_NAME}}</i> с помощью команды /restart."
pl_commands_list = "Команды плагина <b><i>{}</i></b>."
pl_author = "Автор"
pl_new = "Пришли мне файл плагина (<code>.py</code>).\n\n" \
         "<b>⚠️ Плагин исполняется с теми же правами, что и бот: он имеет доступ к golden_key, " \
         "токену Telegram и файловой системе. Ставь только тот код, который прочитал сам " \
         "или получил из доверенного источника.</b>"
...
cmd_restart = "перезапустить {{BOT_SHORT_NAME}}"
cmd_power_off = "выключить {{BOT_SHORT_NAME}}"
...
# --- 4FunPay ---
update_manual_help = """ℹ️ Установлена версия <code>{}</code>.\n
🔄 Встроенное самообновление отключено намеренно: код обновляется через git,
чтобы обновление нельзя было получить из постороннего источника.\n
<b>Как обновиться на сервере:</b>
...
golden_key_from_env = "🔒 golden_key задан переменной окружения <code>{}</code>.\n\n" \
                      "Смена через бота не подействует: значение из окружения перекроет конфиг " \
                      "при следующем запуске.\n\n" \
                      "Меняй его там, где задаётся переменная - в systemd-юните, файле .env " \
                      "или docker compose, - затем /restart."
auth_expired = "🔑 <b>FunPay не принимает golden_key.</b>\n\n" \
                      "Cookie истёк или отозван. Обычные причины: смена пароля на FunPay, " \
                      "выход из всех сессий, вход с другого устройства.\n\n" \
                      "<b>Что делать:</b>\n" \
                      "1. Зайди на funpay.com в браузере.\n" \
                      "2. F12 -> Application -> Cookies -> https://funpay.com -> golden_key.\n" \
                      "3. Скопируй значение и отправь мне команду /golden_key\n\n" \
                      "Пока ключ не обновлён, автовыдача и автоответы не работают. " \
                      "Попытки подключения продолжаются каждые {} сек."
auth_restored = "✅ Подключение к FunPay восстановлено. Аккаунт: <code>{}</code>"
confirm_password_request = "🔐 <b>Требуется подтверждение</b>\n\n" \
                          "Действие: <b>{}</b>\n\n" \
                          "Отправь пароль от панели управления - тот же, которым ты входил.\n" \
                          "Запрос действует {} сек."
confirm_wrong_password = "❌ Пароль неверный. Осталось попыток: <b>{}</b>."
confirm_too_many_attempts = "🚫 Попытки исчерпаны, запрос отменён. Начни действие заново."
confirm_expired = "⌛ Срок подтверждения истёк. Начни действие заново."
confirm_nothing_pending = "🤷 Нечего подтверждать."
confirm_action_change_golden_key = "смена golden_key"
confirm_action_power_off = "выключение бота"
confirm_action_restore_backup = "восстановление из резервной копии"
confirm_action_upload_plugin = "установка плагина"
confirm_action_change_authorized_users = "изменение списка доступа"
